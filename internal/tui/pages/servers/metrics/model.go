...
func New(source *model.RuntimeGraph, lang config.ServiceLang) Model {
	return NewWithTheme(source, lang, styles.Zakura)
}

// NewWithTheme creates a metrics panel using theme.
func NewWithTheme(source *model.RuntimeGraph, lang config.ServiceLang, theme styles.Theme) Model {
	result := Model{
		lang:    lang,
		...
		style: Style{
			...
			CPU:       lipgloss.NewStyle().Foreground(theme.CPU),
